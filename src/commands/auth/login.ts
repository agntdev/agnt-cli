...
export default class AuthLogin extends Command {
  ...
  async run(): Promise<void> {
    ...
    if (!process.stdin.isTTY) {
      this.error(
        "Non-interactive environment detected. Use --token to pass credentials directly.\n  Example: agnt auth login --token amk_xxxx",
        { exit: 2 },
      );
    }
    ...
    this.log(`  Verification code: ${session.verification_code}`);
    ...
    while (Date.now() < deadline) {
      ...
      try {
        ...
        if (res.status === 410) {
          ...
          this.error(
            `Session expired. Run ${this.config.bin} auth login again.${text ? " " + text : ""}`,
            { exit: 1 },
          );
        }
